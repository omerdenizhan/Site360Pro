...
class DashboardController extends Controller
{
    public function __invoke(): View
    {
        ...
        $dues = Due::query()
            ...
            ->limit(10)
            ->get()
            ->sortBy(fn (Due $due) => (int) $due->apartment->number)
            ->values();

        $followItems = Due::query()
            ->with([
                'apartment.buildingBlock',
                'apartment.activeResident',
            ...
            ->when(
                ...
                fn ($query) => $query->whereHas(
                    'apartment',
                    fn ($apartment) => $apartment->where('building_block_id', $blockId)
                )
            )
            ->get()
            ...
            ->filter(fn (array $item) => $item['paid'] <= 0)
            ->sortByDesc('pending')
            ->values();
        ...
        $otherIncomeTotal = Income::query()->where('site_id', $site->id)->sum('amount');
        ...
        return view('dashboard', [
            ...
            'cashBalance' => (float) $incomeTotal + (float) $otherIncomeTotal - (float) $expenseTotal,
            ...
            'recentPayments' => Payment::with(['bankTransaction', 'due.apartment.buildingBlock', 'due.apartment.activeResident'])
                ...
                ->limit(5)
                ->get(),
            'incomes' => Income::query()->where('site_id', $site->id)->latest()->take(5)->get(),
