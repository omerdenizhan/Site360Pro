...
<body>
    <div class="header">
        <div class="left">
            <h1>{{ $site->name }}</h1>
            <br>
            <h3 class="fw-bold">{{ $payment->due->apartment->buildingBlock->name }}</h3>
    ...
    <div class="box">
        ...
        <strong>Daire No.:</strong> {{ $payment->due->apartment->number }}<br>
        <strong>Ödeme Tarihi:</strong> {{ $payment->paid_at->format('d.m.Y H:i') }}<br>
        <strong>Ödeme Yöntemi:</strong> {{ strtoupper($payment->method) }}
    </div>
